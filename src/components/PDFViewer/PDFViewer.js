...
import Style from "assets/jss/material-kit-react/components/pdfViewerStyle.js";
...
const ZoomNavigator = ({ config, setConfig }) => {
  ...
  return (
    ...
    >
      <Grid item>
        <Tooltip title="Max/Min" placement="left">
          <Fab
            disableRipple
            color="primary"
            aria-label="add"
...
const PagesNavigator = ({
  ...
  return (
    <Grid
      ...
      spacing={2}
    >
      <Grid item xs={12} sm={3} md={3} className={containerFilename}>
        <Icon className={iconFilename}></Icon>
        <span className={title}>{fileNameProcessed}</span>
      </Grid>

      {fullLoading && numPages > 10 && (
        <Grid item xs={12} sm={6} md={6} className={containerPagination}>
          {`Page ${page * 10 + 1} to ${
            numPages <= 10
              ? numPages
              : (page + 1) * 10 > numPages
      ...
      <Grid item xs={12} sm={3} md={3}>
        ...
        >
          {numPages > 10 && (
            <div>
              <Tooltip title="10 previous pages" placement="bottom">
                <Fab disableRipple onClick={() => handlePage("previous")}>
                  <Icon>navigate_before</Icon>
                </Fab>
              </Tooltip>

              <Tooltip title="Next 10 pages" placement="bottom">
...
const PDFViewer = ({ file, filename, onClose, isClose, isViewSubFile }) => {
  const classes = Style();
    ...
    <>
      <Grid
        ...
        className={
          ...
          " " +
          (isViewSubFile
            ? classNames(classes.containerSubFile, classes.scroll)
            : null)
        }
      >
        <PagesNavigator
        ...
        <Grid
          ...
          className={
            ...
            " " +
            (isViewSubFile
              ? classNames(classes.contentSubFile, classes.scroll)
              : null)
          }
        >
          <Grid item xs={12}>

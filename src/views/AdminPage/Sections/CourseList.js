...
export default function CourseList() {
  ...
  const { data, success } = usePromiseResult(() => getAllCourse());
      ...
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          ...
          <CardBody>
            {success ? (
              <CourseTable
                labelSearch="Search Course"
                tableHeaderColor="primary"
                tableHead={["ID", "Name", "Description", "Credit", "Materials"]}
                tableData={myData.map((ele, index) => {
                  const number = ele.materials.length;
                  return [
                    ++index,
                    ele.name,
                    ele.description,
                    ele.credits,
                    number <= 1
                      ? `${number} file attachment`
                      : `${number} file attachments`,
                  ];
                })}
